...
import { Clock, Users, Award, Zap, CheckCircle, Star, Gift, Loader2, CreditCard, Smartphone } from 'lucide-react';
...
export function CourseEnroll({ course }: CourseEnrollProps) {
  ...
  return (
    <section className="py-8 bg-gradient-to-r from-green-50 to-blue-50 dark:from-green-950/20 dark:to-blue-950/20">
        ...
        <Card className="max-w-4xl mx-auto overflow-hidden shadow-xl border-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm">
          <CardContent className="p-0">
            ...
            <div className="bg-gradient-to-r from-primary to-primary/80 p-6 text-white text-center">
              ...
              <p className="text-white/90 mt-1">One-time payment for lifetime access.</p>
            </div>
            ...
            <div className="p-6">
              ...
              {!isFree && session && (
                <div className="mb-6">
                  <Label className="font-semibold text-foreground mb-3 block">1. Select Payment Method</Label>
                  <div className="grid grid-cols-2 gap-4">
                    <Button
                      variant={paymentMethod === 'card' ? 'default' : 'outline'}
                      onClick={() => setPaymentMethod('card')}
                      className="py-6 text-base h-auto"
                    >
                    ...
                    <Button
                      ...
                      className="py-6 text-base h-auto"
                    >
                      <Smartphone className="mr-2 h-5 w-5" /> Mobile Wallet
                    </Button>
                  </div>
                  {paymentMethod === 'wallet' && (
                    <div className="mt-4 animate-in fade-in duration-300">
                      <Label htmlFor="walletNumber" className="text-sm font-medium">Wallet Mobile Number</Label>
                      <Input
                        id="walletNumber"
                        type="tel"
              ...
              <div className="mb-6">
                {session ? (
                  <Button
                    ...
                    className="w-full bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 text-white font-bold py-4 text-lg shadow-lg h-auto"
                  >
                    {initiatePayment.isPending ? (
                      <><Loader2 className="h-5 w-5 mr-2 animate-spin" /> Processing...</>
                    ) : (
                      <><Zap className="h-5 w-5 mr-2" /> {isFree ? 'Start Learning Now' : 'Complete Enrollment'}</>
                    )}
                  </Button>
                ) : (
